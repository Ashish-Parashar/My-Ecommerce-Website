...
function Userdashboard() {
    ...
    const [Catacitve, setCatactive] = useState(0)
    ...
    const categoryName = ['All', ...new Set(products.map((item) => item.category.name ? item.category.name: item.category))]

    const filtercat = Productcategory === 'All' ? products :
        products.filter((item) => item.category.name === Productcategory)
    ...
    return (
        <>
            ...
            <div className=' bg-white dark:bg-gray-800 flex flex-wrap gap-3 py-4 p-2' >
                {
                    categoryName.map((item, index) =>
                        <button className="px-4 py-2 rounded-md bg-blue-500  text-white hover:bg-blue-700 transition-all duration-200 text-sm font-semibold shadow-sm"
                            style={{
                                backgroundColor: Catacitve === index ? '#1D4ED8' : null
                            }}
                            onClick={() => {
